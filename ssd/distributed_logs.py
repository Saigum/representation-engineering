...
def run_job(config_file, gpu_id,log_dir):
    """
    Runs training and inference for a single config file on a specific GPU.
    """
    ...
    env = os.environ.copy()
    # CRITICAL: Set CUDA_VISIBLE_DEVICES for this specific subprocess
    # This tells CUDA which GPU to use (e.g., '0', '1', '2', '3')
    ...
    try:
        ...
        return (config_file, f"Failed with code {e.returncode}")
    except Exception as e:
        print(f"!!! [GPU {gpu_id}] PYTHON ERROR on job: {tag}: {e}")
        return (config_file, f"Failed with exception: {e}")
